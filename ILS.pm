...
sub checkout {
	...
	if (!$patron) {
		...
		if ($circ->ok){
			...
			# Since $item->{due_date} might be undef, we need to
			# delete it first otherwise we encounter "Modification of
			# non-creatable hash value attempted, subscript "_" at
			# ILS.pm line 181." error
...
sub renew {
	...
	$circ = new ILS::Transaction::Renew;
	$circ->patron($patron = new ILS::Patron $patron_id);
	$circ->item($item = new ILS::Item $item_id);
...
sub renew_all {
	# This function basically has to loop over all items in the
	# patrons possesion. However, unlike renew, it sets the arrays
	# `renewed` and `unrenewed` to the item barcodes to transport
	# success status and refresh the SIP display accoringly. Thus `ok`
	# is always set to 1.
	my ($self, $patron_id, $patron_pwd, $fee_ack) = @_;
	my ($patron, $item_id);
	my $circ;

	$circ = new ILS::Transaction::RenewAll;
	$circ->patron($patron = new ILS::Patron $patron_id);
